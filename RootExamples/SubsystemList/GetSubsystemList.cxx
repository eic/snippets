// ============================================================================
//! \file   GetSubsystemList.cxx 
//! \author Derek Anderson
//! \date   06.08.2026
// ----------------------------------------------------------------------------
//! \brief
//!   Short macro to illustrate how to get a 
//!   list of subsystem IDs from an edm4eic::
//!   ReconstructedParticle.
//!
//! \usage
//!   Depends on DD4hep, PODIO, EDM4eic, and EDM4hep.
//!   Can be run out of the box inside eic-shell.
//!
//!       $ root -b -q GetSubsystemList.cxx
// ============================================================================

#define GetSubsytemList_cxx
...
// default options
static const std::string IFileDefault = "root://dtn-eic.jlab.org//volatile/eic/EPIC/RECO/26.04.0/epic_craterlake/DIS/pythia6.428-1.0/NC/noRad/ep/10x130/q2_10to100/pythia6.428-1.0_NC_noRad_ep_10x130_q2_10to100_ab.0625.eicrecon.edm4eic.root";
static const std::string IReadDefault = "HcalBarrelHits";
static const std::string IParsDefault = "ReconstructedParticles";
...
struct Options {
  ...
  std::string ipars;  // recostructed particle collection to process
  std::size_t npars;  // max no. of particles to print out
} DefaultOptions = {
  IFileDefault,
  IParsDefault,
  5
...
// ============================================================================
//! Helper to extract set of subsystem IDs
// ============================================================================
/*! TODO replace with EDM4eic utility when ready */
std::set<uint32_t> get_subsystem_list(
    const edm4eic::ReconstructedParticle& particle,
    const BitFieldCoder* decoder
) {

  // TODO
  //   1. Loop through clusters, hits. Extract Cell IDs from there.
  //   2. Loop through tracks, ..., hits. Extract Cell IDs from there.
  //   3. ??? PID ???

  std::set<uint32_t> list = {111};
  return list;
...
// ============================================================================
void GetSubsystemList(const Options& opt = DefaultOptions) {
