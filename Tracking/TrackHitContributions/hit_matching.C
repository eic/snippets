...
void hit_matching(){
	...
	//1) All associated RecHits for events w/ 1 track
	//2) All associated measurement RecHits for events w/ 1 track
	//3) All associated outlier RecHits for events w/ 1 track
	int nBins = rec_coll_names.size();

    	TH1F *h8a = new TH1F("h8a","Events with 1 reconstructed track", nBins, 0, nBins); //Associated digitized hits
	h8a->SetLineWidth(3);h8a->SetLineColor(kBlue);
	h8a->GetYaxis()->SetTitle("Counts");h8a->GetYaxis()->CenterTitle();

	TH1F *h8b = new TH1F("h8b","Events with 1 reconstructed track", nBins, 0, nBins); //Associated track measurement hits
        h8b->SetFillColor(kGreen+2);h8b->SetFillStyle(3004);h8b->SetLineWidth(0);
        h8b->GetYaxis()->SetTitle("Counts");h8b->GetYaxis()->CenterTitle();

	TH1F *h8c = new TH1F("h8c","Events with 1 reconstructed track", nBins, 0, nBins); //Associated outlier hits
	...
	//Number of RecHits per collection
        ...
        TH1F *h9a = new TH1F("h9a","", nBins, 0, nBins); //Associated RecHits
        h9a->SetLineWidth(3);h9a->SetLineColor(kBlue);
        h9a->GetYaxis()->SetTitle("Counts");h9a->GetYaxis()->CenterTitle();

        TH1F *h9b = new TH1F("h9b","", nBins, 0, nBins); //Associated Measurement2D
	...
	for( unsigned int ievent = 0; ievent < nevents; ievent++){
		...
		for(int icoll = 0; icoll < sim_coll_names.size(); icoll++){
			...
			for(int ihit = 0; ihit<num_simhits; ihit++){
				...
				//For Barrel MPGDs, a single SimHit now usually gets converted into 2 Raw/Rec Hits
				//So, we add an additional count for those to keep the numbers consistent
				if( coll_name=="MPGDBarrelHits" || coll_name=="OuterMPGDBarrelHits"){
					if(mc_status==1 && quality==0) matched_simhits++;
                                	tot_simhits++;					
				}
				...
				for (const auto raw_hit_assoc : raw_hit_assocs) {
					if (raw_hit_assoc.getRawHit() == raw_hit) {
						...
						if(mc_status==1 && quality==0){ 
							...
							//If there is 1 track, fill histogram of collection RecHits
							auto& track_coll = f.get<edm4eic::TrackCollection>(trk_coll);
                					auto num_tracks = track_coll.size();

							if(num_tracks == 1 && (index_map.find(coll_name) != index_map.end()) ){
