...
## standard performance scan (pTDR style)
Folder __pTDR__: a complete workflow to generate simulation files, calculate tracking efficiency and resolutions bin by bin, and plot results v.s. momentum.
...
* plot the number of sim hits/measurements/outliers per detector (the example shows only barrel trackers, endcap plots can be produced in a similar fashion).
