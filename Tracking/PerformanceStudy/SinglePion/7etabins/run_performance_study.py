...
def plot_resol(pion,params):
    ...
    ax = axs.flat[i]
    # obs    = 'theta'
    rec   = np.abs( 1./np.array(params['qOverP']) )
...
## degrees of 3,6,10,40,140,170,174,177 correspond to eta bin of -3.5,-3.0,-2.5,-1,1,2.5,3.0,3.5
def performance_plot(fname,rootfile_path,eff_eta_bins=np.linspace(-4,4,21),resol_deg_bins=np.array([3,5.5,10,40,140,170,174.5,177]),pid=-211, out_dir="."):
    out_dir=out_dir+"/"
    ## read events tree
    tag     = fname.split('/')[-1][:-5]
...
if __name__ == "__main__":
    ...
    # resol_deg_bins =[0] 
    resol_deg_bins = np.array([3.4595, 5.7005, 9.3848, 40.3907, 139.5998, 170.6082, 174.2956, 176.5350])
    ...
    for mom in mom_list:
        for ii in np.arange(len(eta_list)-1):
            filename = dirname+f"rec_{setting}_eta_{eta_list[ii]}_{eta_list[ii+1]}_{mom}GeV_{nev}.root"
            print(filename)
