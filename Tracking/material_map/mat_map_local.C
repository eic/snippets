...
void Draw_ratio(TCanvas* c, TProfile* h1, TProfile* h2, TLegend* leg, std::string name){
  ...
  c->Print( (name+"/Ratio_Val_geant_mat_X0_"+name_axis+".png").c_str());
...
void mat_map_local(std::string Val = "", std::string geantino = "", std::string name = ""){
  ...
  TH2F * comp_X0 = new TH2F("comp_X0","comp_X0",160,-4,4,160,-4,4);
    ...
    TH2F * mat_map1 = new TH2F("mat_map1","",2000,-3000,3000,1000,-10,1000);
    TH2F * mat_map2 = new TH2F("mat_map2","",2000,-3000,3000,1000,-10,1000);
  ...
  eta_4n->SetLineColor(kRed);
    ...
    // 2D map for ACTS input
    Val_file->Add(Val.c_str());
    TCanvas *VM = new TCanvas("VM","Validation Map") ;

    Val_file->Draw("sqrt(mat_x**2+mat_y**2):mat_z>>mat_map1","abs(mat_z)<5000");
    ...
    Val_file->Add(Val.c_str());
    TCanvas *VM_eta = new TCanvas("VM_eta","Validation Map") ;
    Val_file->Draw("sqrt(mat_x**2+mat_y**2):v_eta","abs(mat_z)<5000");
    VM_eta->SetGrid();
    VM_eta->Print( (name+"/Val_mat_map_eta.png").c_str());
    ...
    geantino_file->Draw("sqrt(mat_x**2+mat_y**2):mat_z>>mat_map2","abs(mat_z)<5000");
